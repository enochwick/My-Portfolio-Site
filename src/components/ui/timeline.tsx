...
export const Timeline = ({
  ...
  return (
    <div className="w-full font-sans md:px-10" ref={containerRef}>
      {(heading || intro) && (
        <div className="mx-auto max-w-7xl px-4 pt-6 md:px-8 lg:px-10">
          {heading && (
        ...
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:gap-10 md:pt-40">
            <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              ...
              <h3 className="jm-display hidden text-xl font-bold text-paper/30 md:block md:pl-20 md:text-5xl">
                {item.title}
              </h3>
            </div>

            <div className="relative w-full pl-20 pr-4 md:pl-4">
              <h3 className="jm-display mb-4 block text-left text-2xl font-bold text-paper/40 md:hidden">
                {item.title}
              </h3>
              {item.content}
            </div>
